...
export class AuthController {
    ...
    ftOAuth() {
        return {
            msg: "this is returned when something is wrong with the guards.;",
        };
    }
    ...
    async ftRedirect(@Req() req, @Res() res: Response) {
        const { url, newToken }: signInReturn = await this.authService.ftSignin(
            req.user
        );

        if (url != undefined) res.cookie("url", url);
        res.cookie("token", newToken, {
            // httpOnly: true,
            // sameSite: 'strict',
        });
        if (req.user.twoFa) res.redirect("http://localhost:3000/2fa");
    ...
    async activate2Fa(@Req() req, @Res() res: Response) {
        ...
        return res.json({
            "msg:": "the QR code for 2fa activation is provided. One more step to have 2fa is remained.",
            token: newToken,
        });
    }
    ...
    async ft_logout(@Req() req, @Res() res: Response) {
        ...
        return res.json({ message: "the logout was successful!" });
    }
    ...
    async deactivate2fa(
        ...
        const newToken = await this.authService.deactivate2fa(
            twoFaDto,
            req.user
        );
        res.cookie("token", newToken, {
