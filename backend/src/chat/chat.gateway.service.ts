...
export class ChatGatewayService {
	...
	getNeededPermissionLevel(user, changeForm: ChangeChatUserStatusDTO): string {
		if (user.owner || user.admin || changeForm.status == "owner") {
			return "owner";
		}
		return "admin";
	}
	...
	async changeAdmin(changeForm: ChangeChatUserStatusDTO) {
		...
		await this.prisma.chat_User.updateMany({
			...
			data: {
				admin: Boolean(changeForm.value),
			},
		});
		this.sendChatUpdate(changeForm.chatId);
	}
	...
	async changeUsersInChatStatus(changeForm: ChangeChatUserStatusDTO) {
		try {
			if (!(await this.IsUserInChat(changeForm.chatId, changeForm.userId))) {
				const socket: Socket = this.getUserSocketFromUserId(changeForm.operatorId);
				socket.emit("error", "User is not in chat");
				return -1;
			}
			if (changeForm.userId == changeForm.operatorId) {
				const socket: Socket = this.getUserSocketFromUserId(changeForm.operatorId);
				socket.emit("error", "You cant change your own status");
				return -1;
			}
			...
			if(!operatorUser || !changeUser){
				const socket: Socket = this.getUserSocketFromUserId(changeForm.operatorId);
				socket.emit("error", "User not found");
				return -1;
			}
			if (changeUser.blocked || operatorUser.blocked) {
				const socket: Socket = this.getUserSocketFromUserId(changeForm.operatorId);
				socket.emit("error", "User is blocked");
				return -1;
			}
			if (
				this.getNeededPermissionLevel(changeUser, changeForm) == "owner" &&
				operatorUser.owner == false
			)
				throw {message: "You dont have permission to change owner"};
			if (
				this.getNeededPermissionLevel(changeUser, changeForm) == "admin" &&
				operatorUser.admin == false
			)
				throw {message: "You dont have permission to change owner"};
			if (changeForm.status == "owner") this.changeOwner(changeForm);
			else if (changeForm.status == "admin") this.changeAdmin(changeForm);
			else if (changeForm.status == "kick") this.kickChatUser(changeForm);
			else if (changeForm.status == "mute") this.muteChatUser(changeForm);
			else if (changeForm.status == "ban") this.banChatUser(changeForm);
			else throw {message: "Wrong status"};
		} catch (error) {
